...
class ComputeIndices:
    ...
    def get_monthly_collection(self,year, month):
        start = ee.Date.fromYMD(year, month, 1)
        end = start.advance(1, 'month')
        return self.collection.filterDate(start, end)

    # Function to compute monthly median
    def get_monthly_median(self, image_collection, year, month):
        start = ee.Date.fromYMD(year, month, 1)
        return image_collection.median().set('system:time_start', start.millis())
    ...
    def add_indices(self, image):
        ...
        # print('TCARI_OSAVI computed')
        # image = self.compute_wdrvi(image)
        # print('WDRVI computed')
        # image = self.compute_gndvi(image)
        # print('GNDVI computed')
    ...
    def computeIndexes(self, start_date, end_date):
        ...
        # Compute the monthly median for each month
        monthly_medians = ee.ImageCollection.fromImages(months.map(lambda date: self.get_monthly_median(self.get_monthly_collection(ee.Date(date).get('year'), ee.Date(date).get('month')), ee.Date(date).get('year'), ee.Date(date).get('month'))))
        ...
        monthly_indices_list = self.image_indexes.toList(self.image_indexes.size())
        # print("******************")
        # print(monthly_indices_list)
